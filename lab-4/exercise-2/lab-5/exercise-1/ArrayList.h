...
template<class E>
struct ArrayList : public AbstractList<E> {
    ...
    ArrayList(){
        ...
        data = new E[capacity];   // Allocate enough space on the heap
    }
    ...
    void append(E value){
        ...
        if (count == capacity){     // By storing the element above, we may no longer have space for at least one more
            ...
            E* temp = new E[capacity];          // Allocate new space

            for (int i = 0; i < oldCapacity; i++){  // Copy elements from old space to new space
                temp[i] = data[i];
            }

            E* oldLocation = data;        // Keep track of where the original storage location was
